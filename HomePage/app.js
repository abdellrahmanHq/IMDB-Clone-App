...
//api website :https://developer.themoviedb.org/docs
//https://api.themoviedb.org/3/discover/movie?api_key=49e3be45df1c1a483b5eb9560e3c73ab&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_watch_monetization_types=flatrate
...
function addMovie(Id) {
    document.getElementById(Id).innerHTML = '<span class="icon-color"><i class="fas fa-check"></i></span>';
    if (!mylist.includes(Id.toString())) {
        mylist.push(Id.toString());
    }
    oldLocalsMov = JSON.parse(localStorage.getItem('MovieArray'));
    if (oldLocalsMov == null) {
        localStorage.setItem('MovieArray', JSON.stringify(mylist));
    } else {
        mylist.forEach(item => {
